...
export default function Edit({match}) {
   ...
   let editWorker =null;
    ...
    useEffect(() => {
        ...
        if(user) {
            ...
            subscribe = db.collection("workersCollection")
                        .onSnapshot((docs) => {
                                docs.forEach((doc) => {
                               editWorker = {...doc.data(), id: doc.id}
                                if(editWorker.workerid.slice(0,29) === match.params.id){
                                    //newWorkers.push(worker);
                                    bindInput(editWorker);
                                    setState(editWorker);
    ...
    function showAddedWorked(){
        resetValue();
        setValidationMessage('Editerea fost realizata cu succes.');
        openModal();
    }
    ...
    let json = require('../add/Specializations.json');
        ...
        <div>
            ...
            <h1 className="display-4">Maistru</h1>
                <form onSubmit={handleSubmit} autoComplete="off" > 
                    ...
                    <button className="btn btn-primary add-btn" onClick={handleEdit}>Editare</button>
                </form>

                <Modal {...modalProps} title="Editare maistru" >
                    <div className="form-group">
